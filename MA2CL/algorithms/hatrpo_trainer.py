...
class HATRPO:
    ...
    def kl_divergence(
        ...
        if mu.grad_fn == None:
            ...
            kl = (
                logstd
                - logstd_old
                + (std_old.pow(2) + (mu_old - mu).pow(2)) / (2.0 * std.pow(2))
                - 0.5
            )
    ...
    def trpo_update(self, sample, update_actor=True):
        ...
        loss_grad = torch.autograd.grad(
            loss, self.policy.actor.parameters(), allow_unused=True
        )
        ...
        fvp = self.fisher_vector_product(
            self.policy.actor,
            obs_batch,
            rnn_states_batch,
            actions_batch,
        ...
        old_actor = Actor(
            self.policy.args, self.policy.obs_space, self.policy.act_space, self.device
        )
