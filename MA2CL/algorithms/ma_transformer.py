...
class Encoder(nn.Module):
    def __init__(
        ...
        if isinstance(self.obs_dim, (list, tuple)):
            self.obs_encoder = CNNLayer(obs_dim, n_embd)
        else:
    ...
    def forward(self, state, obs):
        rep = self.get_rep(state, obs)
        v_loc = self.head(rep)
        return v_loc, rep

    def get_rep(self, state, obs, cnn_rep=False):
...
class Decoder(nn.Module):
    ...
    def get_rep(self, action, obs_rep, obs):
        if self.dec_actor:
            if self.share_actor:
                rep = self.mlp[:-2](obs)
            else:
                rep = []
                for n in range(len(self.mlp)):
                    rep_n = self.mlp[n][:-2](obs[:, n, :])
                    rep.append(rep_n)
                rep = torch.stack(rep, dim=1)
        else:
            action_embeddings = self.action_encoder(action)
            rep = self.ln(action_embeddings)
            for block in self.blocks:
                rep = block(rep, obs_rep)
