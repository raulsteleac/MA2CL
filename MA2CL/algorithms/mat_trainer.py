...
from MA2CL.algorithms.utils.jpr import JPR
...
class MATTrainer:
    ...
    def ppo_update(self, sample):
        ...
        if self._use_policy_active_masks:
            policy_loss = (
                policy_loss * active_masks_batch
        ...
        if self._use_policy_active_masks and active_masks_batch is not None:
            dist_entropy = (
    ...
    def maska_update(self, sample):
        (obs_batch, obs_label_batch, non_mask, action_batch,action_label_batch) = sample
        obs_batch = check(obs_batch).to(**self.tpdv)
        obs_label_batch = check(obs_label_batch).to(**self.tpdv)
        action_batch = check(action_batch).to(**self.tpdv)
        action_label_batch = check(action_label_batch).to(**self.tpdv)

        loss = self.mask_agent.get_loss(obs_batch, obs_label_batch, non_mask, action_batch, action_label_batch)
            ...
            for sample in data_generator:
                ...
                if self._use_jpr and self.num_updates % self.jpr_update_freq == 0:
                    jpr_loss, jpr_grad = self.jpr_update(
                        buffer.sample_jpr(self.jpr_bsz, self.jpr_jumps)
                    )
                    ...
                    if self.use_linear_jprlr_decay:
                        update_linear_schedule(
                            self.jpr_optimizer, episode, episodes, self.args.jpr_lr
                        )
