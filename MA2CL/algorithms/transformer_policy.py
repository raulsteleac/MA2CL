...
class TransformerPolicy:
    ...
    def __init__(
        ...
        params_volume = get_model_params_volume(self.transformer)
        print("=" * 60)
        print(f"Total params of MAT: {params_volume[0]}")
        print(f"Trainable params of MAT: {params_volume[1]}")
        print(f"Non-trainable params of MAT: {params_volume[2]}")
        print("=" * 60)
    ...
    def evaluate_actions(
        ...
        action_log_probs, values, entropy = self.transformer(
            cent_obs, obs, action, available_actions=available_actions
        )
    ...
    def save(self, save_dir, episode):
        torch.save(
            self.transformer.state_dict(), f"{save_dir}/transformer_{episode}.pt"
        )
        torch.save(self.transformer.state_dict(), f"{save_dir}/transformer.pt")
