...
class SMACMultiRunner(Runner):
    """Runner class to perform training, evaluation. and data collection for SMAC. See parent class for details."""

    def __init__(self, config):
        super(SMACMultiRunner, self).__init__(config)
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                print(
                    "\n Multi-Maps Algo {} Exp {} Seed {} updates {}/{} episodes, total num timesteps {}/{}, FPS_tmp {}/ FPS {}, remaining_time {:.1f}h/{:.1f}h.\n".format(
                        self.algorithm_name,
        ...
        for eval_env in self.eval_envs:
            ...
            while True:
                ...
                if eval_episode >= self.all_args.eval_episodes:
                    ...
                    eval_env_infos = {
                        eval_map + "/eval_average_episode_rewards": eval_episode_rewards
                    }
                    self.log_env(eval_env_infos, total_num_steps)
                    eval_win_rate = eval_battles_won / eval_episode
                    print(eval_map + " eval win rate is {}.".format(eval_win_rate))
                    self.writter.add_scalars(
                        eval_map + "/eval_win_rate",
                        {eval_map + "/eval_win_rate": eval_win_rate},
                        total_num_steps,
                    )
                    break
    ...
    def eval(self, total_num_steps):
        ...
        while True:
            ...
            for eval_i in range(self.n_eval_rollout_threads):
                ...
                if (
                    eval_dones_env[eval_i]
                    and eval_episode[map_name] < self.all_args.eval_episodes
                ):
            ...
            if (
                np.array(list(eval_episode.values())) == self.all_args.eval_episodes
            ).all():
                break
        ...
        for eval_map in eval_total_reward.keys():
            ...
            self.writter.add_scalars(
                eval_map + "/eval_average_episode_rewards",
            ...
            self.writter.add_scalars(
                eval_map + "/eval_win_rate",
                {eval_map + "/eval_win_rate": aver_eval_winrate},
                total_num_steps,
            )
