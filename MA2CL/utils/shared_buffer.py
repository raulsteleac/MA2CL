...
class SharedReplayBuffer(object):
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        if self._use_proper_time_limits:
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        if "mat" in self.algo or "major" == self.algo or "mask" in self.algo:
    ...
    def feed_forward_generator_transformer(
        ...
        for indices in sampler:
            ...
            if self.use_share_obs:
                ...
                if (
                    len(self.share_obs.shape) > 4
                    and self.share_obs.shape[-1] > self.image_size
                ):
                    share_obs_batch = random_crop(share_obs_batch, self.image_size)
            else:
                share_obs_batch = None
            obs_batch = obs[indices].reshape(-1, *obs.shape[2:])
            if len(self.obs.shape) > 4 and self.obs.shape[-1] > self.image_size:
                obs_batch = random_crop(obs_batch, self.image_size)
            ...
            if self.env_name == "drone" and "ppo" in self.algo:
                BxTxN, C, H, W = obs_batch.shape
                N = self.num_agents
                BxT = BxTxN // N
                share_obs_batch = (
                    obs_batch.reshape(BxT, 1, N * C, H, W)
                    .repeat(N, axis=1)
                    .reshape(BxTxN, N * C, H, W)
                )
    ...
    def feed_forward_generator(
        ...
        for indices in sampler:
            ...
            if self.use_share_obs == False and self.env_name == "drone" and "ppo" in self.algo:
                BxTxN, C, H, W = obs_batch.shape
                N = self.num_agents
                BxT = BxTxN // N
                share_obs_batch = (
    ...
    def sample_jpr(self, mini_batch_size=None, jumps=1):
        """
        Yield training data for MLP policies.
        :param advantages: (np.ndarray) advantage estimates.
        :param num_mini_batch: (int) number of minibatches to split the batch into.
        ...
        # idxs_act = np.lib.stride_tricks.sliding_window_view(idxs, jumps, axis=0)
        # idxs_act = idxs_act.reshape(-1, jumps)

        # TAG we add this to avoid sampling the episode boundaries
        active_masks = self.active_masks[:-1].reshape(-1, *self.active_masks.shape[2:])
        active_masks = active_masks[idxs_obs]
        valid_idxs = np.where((active_masks.mean(axis=(1, 2)) == 1).reshape(-1))[0]
        idxs_obs = idxs_obs[valid_idxs]

        # keep (num_agent, dim)
        if self.use_share_obs:
            share_obs = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[2:])
            share_obs = share_obs[idxs_obs]
        obs = self.obs[:-1].reshape(-1, *self.obs.shape[2:])
        obs = obs[idxs_obs]
        actions = self.actions.reshape(-1, *self.actions.shape[2:])
        # actions = actions[idxs_act]
        actions = actions[idxs_obs]

        indices = torch.randperm(batch_size).split(mini_batch_size or batch_size)[0]
        if self.use_share_obs:
            share_obs_batch = share_obs[indices.numpy()]
        obs_batch = obs[indices.numpy()]
        actions_batch = actions[indices.numpy()]
        return share_obs_batch, obs_batch, actions_batch
    
    def sample_pre(self, mini_batch_size ,jumps, MASK_AGENT):
        """
        Yield training data for MLP policies.
        :param advantages: (np.ndarray) advantage estimates.
        :param num_mini_batch: (int) number of minibatches to split the batch into.
        ...
        valid_idxs = np.where((active_masks.mean(axis=(1, 2)) == 1).reshape(-1))[0]
        idxs_obs = idxs_obs[valid_idxs]
        batch_size = idxs_obs.shape[0]
        
        indices = torch.randperm(batch_size).split(mini_batch_size or batch_size)[0]
        idxs_obs = idxs_obs[indices.numpy()]
        idxs_obs = idxs_obs[:,[0,-1]]

        # keep (num_agent, dim)
        # if self.use_share_obs:
        #     share_obs_batch = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[2:])[idxs_obs]
        obs_batch = self.obs[:-1].reshape(-1, *self.obs.shape[2:])[idxs_obs]
        actions_batch = self.actions.reshape(-1, *self.actions.shape[2:])[idxs_obs]

        
        non_masked = np.zeros((obs_batch.shape[0], self.num_agents), dtype=np.bool)
        obs_label = obs_batch[:,1]
        actions_label = actions_batch[:,1]
        obs, actions = MASK_AGENT.mask_pre(obs_batch, actions_batch, non_masked)
        
        return obs, obs_label, non_masked, actions , actions_label

    # def sample_maska(self, maska_ratio=0.8, maska_bsz=None, maska_type="zero",shuffle_agent=False):
    #     """
    #     mask choices:
    #         all mask
    #         partial mask
    #         no mask
    #         substitute with before observation
    #     Yield training data for MLP policies.
    #     :param advantages: (np.ndarray) advantage estimates.
    #     :param num_mini_batch: (int) number of minibatches to split the batch into.
    #     :param mini_batch_size: (int) number of samples in each minibatch.
    ...
    #     # keep (num_agent, dim)
    #     if self.use_share_obs:
    #         share_obs = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[2:])[indices]  # (batch_size , num_agents , obs_size)
    #     obs = self.obs[:-1].reshape(-1, *self.obs.shape[2:])[indices]
    #     actions = self.actions.reshape(-1, *self.actions.shape[2:])[indices]

    #     # if use shuffle agents
    #     if shuffle_agent:
    #         rows, cols = _shuffle_agent_grid(maska_bsz, self.num_agents)
    #         if self.use_share_obs:
    #             share_obs = share_obs[rows, cols]
    #         obs = obs[rows, cols]
    #         actions = actions[rows, cols]
    ...
    #     # TODO mask agents
    #     obs_label = obs
    #     non_masked = np.zeros((obs.shape[0], self.num_agents), dtype=np.bool)
    #     # obs = MaskAgent.mask_obs(obs_label, non_masked, maska_ratio, maska_type)
    #     # [L,T,N,Dim]-->[L*T,N,Dim]-->[index,N,Dim]-->[index*N, Dim]
    #     if self.use_share_obs:
    #         if (len(self.share_obs.shape) > 4 and self.share_obs.shape[-1] > self.image_size):
    #             share_obs = share_obs.reshape(-1, *share_obs.shape[2:])
                
    #             share_obs = random_crop(share_obs, self.image_size)
                
    #             share_obs = share_obs.reshape(-1, self.num_agents, *share_obs.shape[1:])
    #     else:
    #         share_obs = None
    #     if len(self.obs.shape) > 4 and self.obs.shape[-1] > self.image_size:
    #         obs = obs.reshape(-1, *obs.shape[2:])
    #         obs_label = obs_label.reshape(-1, *obs.shape[2:])
            
    #         obs = random_crop(obs, self.image_size)
    #         obs_label = random_crop(obs_label, self.image_size)
            
    #         obs = obs.reshape(-1, self.num_agents, *obs.shape[1:])
    #         obs_label = obs_label.reshape(-1, self.num_agents, *obs_label.shape[1:])

    #     return share_obs, obs, obs_label, non_masked, actions , actions
